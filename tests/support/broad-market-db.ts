...
/** Bounded fixture seeding: one statement per `size` rows keeps every insert
 * short instead of one multi-second statement holding locks and WAL. */
export async function seedBatches(
  lo: number,
  hi: number,
  size: number,
  run: (lo: number, hi: number) => Promise<unknown>,
) {
  for (let start = lo; start <= hi; start += size)
    await run(start, Math.min(start + size - 1, hi));
}
...
export async function marketDatabase() {
  ...
  for (const [lo, hi] of [
    ...
    await seedBatches(lo, hi, 1000, (from, until) =>
      db.query(
