...
The podium's `#N · covered pools` label, the accounting badge on podium cards and rows, the `N eligible / N excluded` positions cell and mobile footnote, the refresh button and the empty state's basis sentence are gone; the positions column shows the count alone. The reserved slots closed up by exactly what left them, measured on the production build: the desktop row drops from 74px to 62px (its tallest cell is now 21px inside 13px padding, level with the screener and creators rows), the desktop podium card from 204px to 183px and its band from 236px to 215px (the badge line was 21px), and the mobile card from 268px to 224px (the footnote was 44px; content ends at 220px). The mobile podium card keeps 120px because its badge shared the value's row. The mobile card's value row is 33px in both states: its caption rule matches the span after the number rather than the last child, because a pending row holds the number alone and the last-child rule shrank it to 11px (16.5px), a first-paint difference that sat below the Pixel 7 viewport on main and only registered once the copy above it went. The podium's card content and the flat list against the export remain a separate item.
