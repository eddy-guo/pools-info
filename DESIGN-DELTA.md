...
Chain refresh disabled also silences the live trade rail, so its own coverage
stamp went unmeasured until `tests/e2e/layout-stability.spec.ts` began serving
`/api/live-trades/` a resolved feed. The stamp's block, age and catalog size now
hold the width of their resolved form (`trade-stream.module.css`), the same
reserved-inline-block rule the leaderboard's numeric spans follow above: a text
run that rewraps inside a height-reserved box still counts as a layout shift.
