...
import { readContracts, canonicalMulticall3Address } from '../../../packages/chain/src/multicall.ts';
const directory = new URL('./', import.meta.url);
const read = (file) => JSON.parse(fs.readFileSync(new URL(file, directory), 'utf8'));
...
const rpc = {
  batch: async (method, params) => params.map((args) => {
    const row = calls.find((entry) => entry.request.method === method &&
...
assert.deepEqual(decoded.supplies, [{token: recorded.token, supplyRaw: supplyRaw.toString(), block}]);
const units = await readContracts(rpc, [{to: recorded.token, data: '0x313ce567'}], block, config);
assert.equal(BigInt(units.results[0]), decimals);
assert.equal(consumed.size, 2);
const receipt = rows.find((row) => row.request.method === 'eth_getTransactionReceipt').response.result;
...
const tokenTransfers = receipt.logs.filter((log) => log.address.toLowerCase() === recorded.token && log.topics[0] === transferTopic);
...
assert.equal(BigInt(calls.find((row) => row.request.params[0].data === '0x70a08231' + '0'.repeat(64)).response.result), 0n);
assert.equal(BigInt(calls.find((row) => row.request.params[0].data === '0x70a08231' + '0'.repeat(60) + 'dead').response.result), 17786n);
for (const name of ['historical-pair.json', 'current-pair.json']) {
  ...
  assert.equal(BigInt(market.fdvWei), BigInt(market.priceWei) * supplyRaw / 10n ** decimals);
...
for (const line of fs.readFileSync(new URL('SHA256SUMS', directory), 'utf8').trim().split('\n')) {
  const [expected, name] = line.split('  ');
  assert.equal(createHash('sha256').update(fs.readFileSync(new URL(name, directory))).digest('hex'), expected, name);
}
console.log('PASS: 11 RPC records; named block unchanged; direct and real Multicall decoder agree; launch mint and dead-address dust reconcile; both retained Pools Info FDV products exact; all evidence hashes match.');
