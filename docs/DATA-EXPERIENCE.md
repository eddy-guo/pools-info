...
- Existing 90-second production bulk-refresh failure needs deployment verification after the new discovery/batching optimization. Local market cache returned a snapshot collected with 93 HTTP requests in about 30 seconds. Do not call production fixed until checked.

## Next data work

Build a resumable, protocol-specific public catalog instead of downloading the whole chain. Discover verified launch events, preserve pool/token/launch references and metadata, and publish search files partitioned by pool/wallet as needed. Backfill histories on demand or offline; retain explicit block coverage and incomplete accounting flags. A future hosted index or Pools backend can implement the same read interfaces. Neither Railway nor Postgres has been provisioned.
