...
export const broadExploreCtes = `, broad_flow AS (
  SELECT pool_id,sum(trades) AS trades,sum(unsupported) AS unsupported,sum(volume_wei) AS volume
    FROM (
      SELECT pool_id,trades,unsupported,volume_wei FROM broad_market_summaries
        WHERE chain_id=4663 AND batch_end<=$2 AND first_timestamp >= $3 AND last_timestamp <= $6
      UNION ALL
      SELECT k.pool_id,k.trades,k.unsupported,k.volume_wei FROM broad_market_buckets k
        ...
        WHERE k.chain_id=4663 AND k.batch_end<=$2 AND k.timestamp >= $3 AND k.timestamp <= $6
          AND (s.first_timestamp<$3 OR s.last_timestamp>$6)
  ...
  FROM catalog p LEFT JOIN broad_flow f USING(pool_id)
  ...
  WHERE p.launch_block BETWEEN $4 AND $2 AND EXISTS(SELECT 1 FROM pool_launch_sources ps
    WHERE ps.chain_id=4663 AND ps.pool_id=p.pool_id AND ps.stream_key='discovery:v2' AND ps.batch_end<=$5)
), metrics AS (
  ...
  FROM catalog p LEFT JOIN deep_metrics a USING(pool_id) LEFT JOIN broad_metrics b USING(pool_id)
)`;
